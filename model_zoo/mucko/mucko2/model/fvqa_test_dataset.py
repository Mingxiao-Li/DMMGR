...
class FvqaTestDataset(Dataset):
    def __init__(self, config, overfit=False, in_memory=True):
        ...
        with open('fvqa_data/test/test_qa_raw.pickle', 'rb') as f:
            qa_raw = pickle.load(f)


        print('loading semantic graph feature...')
        sem_data = np.load('fvqa_data/test/semantic_graph_test_feature.npz')
        ...
        fact_data = np.load('fvqa_data/test/fact_graph_test_feature.npz')
