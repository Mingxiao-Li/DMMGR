...
class GATLangGraph(nn.Module):
    ...
    def __init__(self, config):
        ...
        for _ in range(config.num_layers):
            ...
            self.graph_modules.append(
                GraphPropageLayer(
                    lang_graph_layer=lang_gat_layer,
    ...
    def forward(self, item):
        ...
        img_edge_rel_len = torch.sum(img_edges != 1, dim=2) # wrong 
        ...
        for b in range(batch_size):
            ...
            node1_list, node2_list = all_node1_id_list,all_node2_id_list
            p_g = dgl.graph(
...
class CrossAtten(nn.Module):
    ...
    def forward(self, lang_graph, graph, prob_graph):
        ...
        for b in range(batch_size):
            ...
            prob_graphs[b].edata["att"] = edge_att_value
...
class GraphPropageLayer(nn.Module):
    def __init__(self, lang_graph_layer, prob_layer, graph_layer, cross_layer):
        super(GraphPropageLayer, self).__init__()
        self.lang_graph_layer = lang_graph_layer
    ...
    def forward(self, lang_graph, prob_graph, graph):
        ...
        for b in range(batch_size):
            ...
            graphs[b].edata["att"] = F.softmax(att_value)
...
class PredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.mid_fc = nn.Linear(config.hidden_size * 3, config.hidden_size*4)
        self.mid_fc2 = nn.Linear(config.hidden_size * 4, config.hidden_size *6)
        self.answer_fc = nn.Linear(config.hidden_size * 6, config.num_answers)
    ...
    def forward(self, graph, question_emb):
        ...
        for g in graphs:
            ...
            node_out = torch.mean(node_out, dim=0)
            edge_out = torch.mean(edge_out, dim=0)
