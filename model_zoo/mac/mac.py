...
from torch.nn.init import kaiming_uniform_, xavier_uniform_, norml
...
class ControlUnit(nn.Module):
    def __init__(self, dim, max_step):
        ...
        self.control_question = linear(dim * 2)
        self.attn = linear(dim, 1)

    def forward(self, step, context, qeustion, control):
        position_aware = self.position_aware[step](qeustion)
...
class MACUnit(nn.Module):
    def __init__(
        self, dim, max_step=12, self_attention=False, memory_gate=False, dropout=0.15
    ):
        super().__init__()
...
class MACNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1024, config.dim, 3, padding=1),
            nn.ELU(),
            nn.Conv2d(config.dim, config.dim, 3, padding=1),
            nn.ELU(),
        )

        self.embed = nn.Embedding(config.n_vocab, config.embed_hidden)
        self.lstm = nn.LSTM(
            config.embed_hidden,
            config.dim,
            batch_first=True,
            bidrectional=config.bidirectional,
        )

        self.lstm_proj = nn.Linear(config.dim * 2, config.dim)
        self.mac = MACUnit(
            config.dim,
            config.max_step,
        ...
        self.classifier = nn.Sequential(
            linear(config.dim * 3, config.dim),
            nn.ELU(),
            linear(config.dim, config.classes),
        )
        self.reset()
    ...
    def forward(
        self, image, question, question_len, knowledge, knowledge_len, dropout=0.15
    ):
        b_size = question.size(0)

        img = self.conv(image)
        img = img.view(b_size, self.config.dim, -1)
